...
func GetOpenPRs(c *UpdateCommand) (pulls []*github.PullRequest, err error) {
	...
	c.Config.Logger.Info("Getting all open PR's targetting the $base branch", "base", c.Config.Base)
	...
	c.Config.Logger.Info("Retrieved all open PR's targetting the $base branch", "base", c.Config.Base, "prCount", len(allPulls))
...
func UpdateOpenPRs(c *UpdateCommand, pulls []*github.PullRequest, targetRef *github.Reference) (err error) {
	...
	for _, pull := range pulls {
		...
		c.Config.Logger.Info("Successfully updated base branch of PR from $base to $target", "base", c.Config.Base, "target", c.Config.Target, "pullNumber", updatedPull.GetNumber(), "pullURL", updatedPull.GetHTMLURL())
	}
	return nil
}
