...
// Config is a struct that contains user inputs
type Config struct {
	Owner  string
	Repo   string
	Base   string
...
func ParseAndValidate(args []string, ui *cli.BasicUi) (c *Config, err error) {
	var (
	...
	if cmd != "inclusify" && cmd != "--version" && cmd != "-version" && cmd != "--help" && cmd != "-help" {
